document.addEventListener('DOMContentLoaded', () => {
  ...
  const fetchLinks = async () => {
    ...
    `).join('');
      ...
      grid.innerHTML = '<p>Could not load extras.</p>';
    }
  };
  ...
  const setupGallery = async () => {
    ...
    const dotsContainer = $('#galleryDots'); // Get the new dots container
    if (!gTrack || !dotsContainer) return;
    ...
    try {
      ...
      const updateUI = () => {
        ...
        dots[gIndex].classList.add('active');
      };
      ...
      $('.gallery-nav.prev')?.addEventListener('click', () => {
        gIndex = (gIndex - 1 + slides.length) % slides.length;
        updateUI();
        startAuto();
      });
      ...
      dotsContainer.addEventListener('click', (e) => {
        ...
        gIndex = Number(targetDot.dataset.index);
        updateUI();
        startAuto();
      });
